...
const getCustomStyles = (formErrors, field) => ({
    control: (provided, state) => ({
        ...provided,
        border: formErrors[field] ? '2px solid #f87171' : state.isFocused ? '2px solid #1a1a1a' : '2px solid #838383',
...
const BuyForm = ({ product, selectedQuantity, selectedTotalPrice }) => {
    ...
    const fetchCitiesByState = async (stateId) => {
        try {
            setLoading(true);
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/store/city/areas/${stateId}`);
            const result = await response.json();
            const cities = result.data.map((city) => ({
                value: city.id,
                label: city.name?.trim(), // Add language support if needed
            }));
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            total_price: selectedTotalPrice < 15 ? selectedTotalPrice + 1.5 : selectedTotalPrice,
        };

        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/store/place/order`, {
                method: "POST",
                headers: {
            ...
            <form onSubmit={handleSubmit}>
                <div className="mb-6" ref={formRefs.fullName}>
                    <input
                        ...
                        className={`w-full px-3 py-3 rounded-lg border-2 ${formErrors.fullName ? 'border-red-500' : 'border-[#838383]'} focus:outline-none focus:ring-1 focus:ring-gray-300 focus:border-gray-900 focus:shadow-lg`}
                        type="text"
                ...
                <div className="mb-6" ref={formRefs.email}>
                    <input
                        ...
                        onChange={handleInputChange}
                        className={`w-full px-3 py-3 rounded-lg border-2 ${formErrors.email ? 'border-red-500' : 'border-[#838383]'} focus:outline-none focus:ring-1 focus:ring-gray-300 focus:border-gray-900 focus:shadow-lg`}
                        type="email"
                ...
                <div className="mb-6 flex flex-col">
                    <div className="flex" ref={formRefs.phone}>
                        ...
                        <input
                            ...
                            onChange={handleInputChange}
                            className={`w-full px-3 py-3 rounded-r-lg border-2 ${formErrors.phone ? 'border-red-500' : 'border-[#838383]'} focus:outline-none focus:ring-1 focus:ring-gray-300 focus:border-gray-900 focus:shadow-lg`}
                ...
                <div className="mb-6" ref={formRefs.address}>
                    <textarea
                        ...
                        onChange={handleInputChange}
                        className={`w-full px-3 py-2 rounded-lg border-2 ${formErrors.address ? 'border-red-500' : 'border-[#838383]'} focus:outline-none focus:ring-1 focus:ring-gray-300 focus:border-gray-900 focus:shadow-lg`}
                        rows={3}
